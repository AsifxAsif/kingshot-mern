...
const PRESET_FIELDS = ['username', 'gameId', 'vault', 'troops', 'buildings', 'heroes', 'heroGear', 'govGear', 'govCharm', 'pets', 'warAcademy', 'widgets', 'misc', 'planner', 'heroShards', 'heroWidgets', 'heroFlowers', 'lockedUpgrades', 'settings', 'pageScores', ];
...
export const createPreset = async (req, res) => {
	try {
		...
		const preset = await Preset.create(orderedPresetDoc({
			userId,
			name,
			username,
			gameId,
...
export const updatePreset = async (req, res) => {
	try {
		...
		if (!preset) return res.status(500).json({
			message: 'Save failed'
		});
		res.json(preset);
	} catch (error) {
