...
export const TRACKED_BUILDINGS = new Set(['Town Center', 'Barracks', 'Stable', 'Range', 'Command Center', 'War Academy', 'Embassy', 'Infirmary', ]);
/**
...
export function collectStepRequirements(steps, selfName = '') {
	...
	for (const step of steps || []) {
		...
		for (const r of arr) {
			...
			if (parsed.level != null && convertLevelToNumeric(parsed.level) > convertLevelToNumeric(prev.level)) {
				best.set(mapKey, parsed);
			}
		}
	}
