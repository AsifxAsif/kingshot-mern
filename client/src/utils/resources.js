...
export const RESOURCE_ALIASES = {
	...
	elitespices: 'elite_spices',
	elite_spice: 'elite_spices',
	silvergoblet: 'silver_goblet',
	silver_goblets: 'silver_goblet',
	copperhorn: 'copper_horn',
	copper_horns: 'copper_horn',
...
const SPEEDUP_KEYS = new Set(['training_speedup', 'building_speedup', 'research_speedup', 'master_speedup', 'general_speedup', ]);
...
export function sumLockedCosts(lockedUpgrades = {}, excludePage = null) {
	...
	for (const [page, costs] of Object.entries(lockedUpgrades || {})) {
		...
		const looksNested = values.length > 0 && values.every((v) => v && typeof v === 'object' && !Array.isArray(v));
		if (looksNested) {
...
export function formatCostLines(costs, vault = {}) {
	...
	const {
		remaining
	} = computeAffordability(norm, vault);
	const lines = [];
	for (const [key, need] of Object.entries(norm)) {
		if (!need) continue;
