...
export function resourceImgFallbacks(id) {
	...
	const alts = {
		elite_spices: ['Elite_Spices.webp', 'elite_spices.webp', 'EliteSpices.webp', 'elite-spices.webp', 'Elite_Spices.png', 'elite_spices.png', 'spices.webp', 'EliteSpices.png', ],
		silver_goblet: ['Silver_Goblet.webp', 'silver_goblet.webp', 'SilverGoblet.webp', 'silver-goblet.webp', 'Silver_Goblet.png', 'silver_goblet.png', 'goblet.webp', 'SilverGoblet.png', ],
		copper_horn: ['Copper_Horn.webp', 'copper_horn.webp', 'CopperHorn.webp', 'copper-horn.webp', 'Copper_Horn.png', 'copper_horn.png', 'horn.webp', 'CopperHorn.png', ],
		general_emblem: ['general_emblem.webp', 'General_Emblem.webp', 'Master_Emblem.webp', 'master_emblem.webp', 'general_emblem.png', 'GeneralEmblem.webp', ],
		master_manuscript: ['master_manuscript.webp', 'Master_Manuscript.webp', 'Masters_Manuscript.webp', 'master_manuscript.png', 'MasterManuscript.webp', ],
		master_speedup: ['master_speedup.webp', 'Master_Speedup.webp', 'master_speedup.png', 'MasterSpeedup.webp', ],
	};
	const list = [];
...
function slugifyMasterPart(s) {
	return String(s || '').toLowerCase().trim().replace(/['’`']/g, '').replace(/&/g, 'and').replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
}
/**
...
export function masterTalentImgFallbacks(idOrName) {
	...
	return [
		...
		asset(`icons/master-${id}-talent.png`), ...masterImgFallbacks(idOrName),
	];
}
...
export function masterSkillImgFallbacks(idOrName, skillName) {
	...
	return [
		...
		asset(`icons/master-${id}-${sk}.png`), ...masterImgFallbacks(idOrName),
	];
}
/** Affinity row icon */
...
export function masterAffinityImgFallbacks(idOrName) {
	...
	return [
		...
		asset(`icons/master-${id}-type.png`), ...masterImgFallbacks(idOrName),
	];
}
