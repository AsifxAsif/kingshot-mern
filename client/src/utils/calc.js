...
export const RESOURCE_ITEMS = [
  { id: 'bread', placeholder: '1.5M', label: 'Bread' },
  { id: 'wood', placeholder: '2.3M', label: 'Wood' },
  { id: 'stone', placeholder: '500K', label: 'Stone' },
  { id: 'iron', placeholder: '125K', label: 'Iron' },
  { id: 'gold', placeholder: '10K', label: 'Gold' },
  { id: 'gems', placeholder: '10000', label: 'Gems' },
  { id: 'truegold', placeholder: '500', label: 'Truegold' },
  { id: 'truegold_dust', placeholder: '1000', label: 'Truegold Dust' },
  { id: 'tempered_truegold', placeholder: '25', label: 'Tempered Truegold' },
  { id: 'hero_xp', placeholder: '1000', label: 'Hero XP' },
  { id: 'stamina', placeholder: '100', label: 'Stamina' },
  { id: 'master_manuscript', placeholder: '50', label: 'Master Manuscript' },
  { id: 'general_emblem', placeholder: '20', label: 'General Emblem' },
  ...
  { id: 'charm_guide', placeholder: '100', label: 'Charm Guide' },
  { id: 'artisans_vision', placeholder: '50', label: "Artisan's Vision" },
  { id: 'gilded_threads', placeholder: '200', label: 'Gilded Threads' },
  { id: 'satin', placeholder: '5000', label: 'Satin' },
  { id: 'mithril', placeholder: '10', label: 'Mithril' },
  { id: 'forge_hammer', placeholder: '50', label: 'Forge Hammer' },
  { id: 'mythic_general_shard', placeholder: '50', label: 'Mythic General Shard' },
  { id: 'epic_general_shard', placeholder: '200', label: 'Epic General Shard' },
  { id: 'rare_general_shard', placeholder: '500', label: 'Rare General Shard' },
  { id: 'building_speedup', placeholder: '2d 14h 35m', label: 'Building Speedup' },
  { id: 'research_speedup', placeholder: '5d 3h 20m', label: 'Research Speedup' },
  { id: 'training_speedup', placeholder: '1d 12h 5m', label: 'Training Speedup' },
...
export function getUpgradeSteps(dataArray, fromLevel, toLevel) {
  ...
  const fromStr = fromLevel === undefined || fromLevel === null || fromLevel === ''
    ? '0'
    : String(fromLevel);
  const toStr = toLevel === undefined || toLevel === null || toLevel === ''
    ? ''
    : String(toLevel);
...
export function applyBuildingSpeedupBuffs(originalSeconds, buffs = {}) {
  ...
  const totalPercent =
    (parseFloat(buffs.buildingPct) || 0) +
    (parseFloat(buffs.wolfPet) || 0) +
...
export function applyTrainingSpeedupBuffs(originalSeconds, buffs = {}) {
  ...
  let remaining = originalSeconds;
  const totalPercent =
    (parseFloat(buffs.trainingPct) || 0) +
    (parseFloat(buffs.kingPos) || 0) +
    (buffs.mobilize ? 30 : 0) +
    (buffs.kvk ? 25 : 0);
  if (totalPercent > 0) remaining = remaining / (1 + totalPercent / 100);
...
export function applyResearchSpeedupBuffs(originalSeconds, buffs = {}) {
  ...
  let remaining = originalSeconds;
  const totalPercent =
    (parseFloat(buffs.researchPct) || 0) +
    (parseFloat(buffs.kingPos) || 0) +
    (buffs.freshIdeas ? 10 : 0);
  if (totalPercent > 0) remaining = remaining / (1 + totalPercent / 100);
  return Math.max(1, Math.ceil(remaining));
}
